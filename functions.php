<?php 
	...
	$user_id = 1;
	// connect to database
	$db = mysqli_connect("localhost", "root", "", "filmverden"); //Ændre den her til vores db-conn.php
	// get post with id 1 from database
	$post_query_result = mysqli_query($db, "SELECT * FROM posts WHERE id=1");
	$post = mysqli_fetch_assoc($post_query_result);

	// Get all comments from database
	$comments_query_result = mysqli_query($db, "SELECT * FROM comments WHERE post_id=" . $post['id'] . " ORDER BY created_at DESC"); //For at slippe for at lægge blogindlæggende ind i databasen kan vi give hver side et id
